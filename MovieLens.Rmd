...
To present the effect of penalty term let's choose a $λ = 20$ and compare the original values and regularized value. The plot show that small the sample are more close to $0$, or closer to mean value, which means a smaller $RMSE$ for that sample.
...
The values of $\hat{b}_i$ and $\hat{b}_u$ that minimize the full model and the code to calculate the $RMSE$ is presented below.
