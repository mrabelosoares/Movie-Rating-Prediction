...
To present the effect of penalty term let's choose a $λ = 20$ and compare the original values and regularized value. The plot show that as small the sample are more close to $0$, or closer to mean value, which means a smaller $RMSE$ for that sample.
...
The `recosystem` had the best accuracy in comparative analysis, and going to be our choice. The algorithm use formatted data to tune the parameters by `$tune`, train by `$train` and test by `$predict`. The tune parameter adjust is: the number of latent factors `dim`, regularization cost for user and movie factors `costp_l2` and `costq_l2`, the step size in gradient descent `lrate`, the number of iterations `niter`.
